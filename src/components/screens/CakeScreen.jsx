...
export default function CakeScreen({ onNext }) {
  ...
  const lightCandle = useCallback(() => {
    ...
    /* Spawn sparkle burst - reduced for performance */
    const isMobile = window.innerWidth < 640
    const count = isMobile ? 4 : sparkleAngles.length
    const burst = sparkleAngles.slice(0, count).map((angle, i) => ({
  ...
  return (
    <div className="bg-[#fff8fc] p-5 sm:p-7 rounded-[40px] sm:rounded-[60px] drop-shadow-xl min-w-48 w-full max-w-110 relative flex flex-col items-center gap-4 my-6 sm:my-10 card-glow overflow-hidden">
      ...
      {[
        ...
        <motion.div
          ...
          style={{ top: d.top, left: d.left, right: d.right, bottom: d.bottom, willChange: 'transform' }}
